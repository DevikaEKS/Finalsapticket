        ...
        <Navbar expand="lg" variant="dark" bg="primary" className="fixed-top">
          ...
          <Navbar.Collapse id="basic-navbar-nav" className={`${isOpen ? 'show' : ''}`}>
            <Nav className="ml-auto">
              <Nav.Link  as={Link} to={`/client/${id}/${com}/${cshort}`}>TicketBooking</Nav.Link>
              <Nav.Link  as={Link} to={`/ticketsummary/${id}`}>Ticket Status</Nav.Link>
      ...
      {!isSmallScreen && (
        <div className={`sidebar ${isOpen ? 'open' : ''}`}>
          ...
          <Nav className="flex-column text-dark">
            <Nav.Link as={Link} to={`/client/${id}/${com}/${cshort}`} className='text-light text-decoration-none ms-4'>TicketBooking</Nav.Link>
            <Nav.Link as={Link} to={`/ticketsummary/${id}`} className='text-light text-decoration-none ms-4'>Ticket Status</Nav.Link>
          </Nav>
        </div>
      )}
    </>
